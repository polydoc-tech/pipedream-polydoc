...
import {
  buildRequestBody,
  mergeDeep,
} from "../components/polydoc/common/build-request-body.mjs";
...
describe("buildRequestBody", () => {
  ...
  it("Template source is wrapped as [template:ID] and carries templateData", () => {
    ...
    expect(r.body.templateData).toEqual({
      invoice_number: "INV-1",
    });
  });
  ...
  it("Cloud Storage delivery is non-binary and sets cloudStorage", () => {
    ...
    expect(r.body.cloudStorage).toEqual({
      presignedUrl: "https://put.example/abc",
    });
  });
  ...
  it("deep-merges advanced JSON into the body (advanced wins)", () => {
    ...
    expect(r.body.pdf.pdfa).toEqual({
      level: "3b",
    });
